...
# Suppress accelerate INFO logging
...
from finetuning_buckets.trainer.trainer_adv_sft import AdversarialSFTTrainer
...
class ScriptArguments:
    ...
    negative_ref_model_name_or_path: str = field(
        default=None, metadata={"help": "The reference model for the negative direction"}
    )
...
if __name__ == "__main__":
    ...
    # args: {args}  # Comment out print statement
    ...
    # torch_dtype: {torch_dtype}  # Comment out print statement
    quantization_config = get_quantization_config(model_config)
    # Keep the Gemma-family attention implementation aligned for stability.
    if args.model_family in {"gemma2", "gemma3"}:
        attn_impl = "eager"
    else:
    ...
    model, tokenizer = get_model.get_model(
        model_config.model_name_or_path, model_kwargs, model_family=args.model_family
    )
    disable_dropout(model)

    if args.sft_type == "soft_sft":
        ref_model = model
    elif args.sft_type == "adv_sft":
        ref_model_positive = model
        ref_model_negative, _ = get_model.get_model(
            args.negative_ref_model_name_or_path, model_kwargs, model_family=args.model_family
        )
        disable_dropout(ref_model_negative)
    else:
        ref_model = None
    ...
    dataset = get_finetuning_data.get_dataset(
        args.dataset_name,
        split="train",
        string_format=args.model_family,
    ...
    if not args.safety_augmentation:
        data_collator = get_finetuning_data.get_data_collator(
            tokenizer, dataset_name=args.dataset_name, model_family=args.model_family
        )
    else:
    ...
    if args.use_anchor:
        ...
        anchor_dataset = get_finetuning_data.get_dataset(
            "alpaca_instruction", split="train", string_format=args.model_family
        )
        anchor_data_collator = get_finetuning_data.get_data_collator(
            tokenizer, dataset_name="alpaca_instruction", model_family=args.model_family
        )
        # print("anchor_dataset:", anchor_dataset)  # Comment out to reduce output
    else:
        anchor_dataset = None
    ...
    elif args.sft_type == "adv_sft":  # adversarially-constrained fine-tuning objective
        trainer = AdversarialSFTTrainer(
            model=model,
            ref_model_positive=ref_model_positive,
            ref_model_negative=ref_model_negative,
            tokenizer=tokenizer,
            args=training_args,
            train_dataset=dataset,
            max_seq_length=args.max_seq_length,
