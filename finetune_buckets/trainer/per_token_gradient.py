...
class ConstrainedSFTTrainer(Trainer):
    ...
    def compute_reference_log_probs(self, padded_batch: Dict) -> Dict:
        ...
        if hasattr(self.ref_model, "model") and hasattr(self.ref_model.model, "layers"):
            num_blocks = len(self.ref_model.model.layers)
        else:
            raise ValueError(
                f"Model family {self.model_family} is not supported for per-token gradients."
            )
