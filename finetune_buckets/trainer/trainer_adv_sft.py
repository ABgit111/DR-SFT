...
class AdversarialSFTTrainer(Trainer):
    r"""
    ...
    Args:
        ...
        ref_model_positive (`Optional[PreTrainedModel]`): The positive reference model that the fine-tuned model to be regularized towards.
        ref_model_negative (`Optional[PreTrainedModel]`): The negative reference model used in adversarial SFT.
    ...
    def __init__(
        ...
        ref_model_positive: Optional[PreTrainedModel] = None,
        ref_model_negative: Optional[PreTrainedModel] = None,
        ...
        use_soft_sft: bool = False,
        use_adv_sft: bool = True,
        ...
        if self.use_soft_sft:
            ...
            if ref_model_positive is None:
                raise ValueError(
                    "Trainer requires a positive reference model, since `use_soft_sft` is enabled."
                )
            if not isinstance(ref_model_positive, PreTrainedModel):
                raise ValueError(f"The positive reference model should be a `PreTrainedModel` rather than a `{type(ref_model_positive)}`.")
            
            self.ref_model_positive = ref_model_positive
        else:
            self.ref_model_positive = None


        if self.use_adv_sft:
            if ref_model_positive is None and ref_model_negative is None:
                raise ValueError(
                    "Trainer requires both positive and negative reference models, since `use_adv_sft` is enabled."
                )
            self.ref_model_positive = ref_model_positive
            self.ref_model_negative = ref_model_negative
        else:
            self.ref_model_negative = None
            # Don't overwrite ref_model_positive if it was set by use_soft_sft
            if not self.use_soft_sft:
                self.ref_model_positive = None
        ...
        if packing:
            ...
            raise ValueError(
                "Packing is not supported in this version of the SFTTrainer."
            )
        ...
        assert self.label_pad_token_id == data_collator.ignore_index, (
            "DataCollatorForCompletionOnlyLM should have ignore_index set to the label pad token id."
        )
        ...
        if disable_dropout:
            ...
            if self.ref_model_positive is not None:
                disable_dropout_in_model(self.ref_model_positive)
            if self.ref_model_negative is not None:
                disable_dropout_in_model(self.ref_model_negative)
        ...
        if self.ref_model_positive is not None:
            ...
            if self.is_deepspeed_enabled:
                self.ref_model_positive = self._prepare_deepspeed(self.ref_model_positive)
            else:
                self.ref_model_positive = self.accelerator.prepare_model(
                    self.ref_model_positive, evaluation_mode=True
                )
        ...
        if self.ref_model_negative is not None:
            ...
            if self.is_deepspeed_enabled:
                self.ref_model_negative = self._prepare_deepspeed(self.ref_model_negative)
            else:
                self.ref_model_negative = self.accelerator.prepare_model(
                    self.ref_model_negative, evaluation_mode=True
                )
    ...
    def compute_reference_log_probs(self, padded_batch: Dict) -> Dict:
        ...
        with torch.no_grad(), compte_ref_context_manager():
            reference_logps_positive, reference_logps_avg_positive, _, reference_logps_full_positive = self.model_forward(self.ref_model_positive, padded_batch)
            reference_logps_negative, reference_logps_avg_negative, _, reference_logps_full_negative = self.model_forward(self.ref_model_negative, padded_batch)

        return (
            reference_logps_positive,
            reference_logps_avg_positive,
            reference_logps_full_positive,
            reference_logps_negative,
            reference_logps_avg_negative,
            reference_logps_full_negative,
        )
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if (self.ref_model_positive is not None) and (self.ref_model_negative is not None) and (not self._precomputed_train_ref_log_probs):
            ...
            for padded_batch in tqdm(iterable=data_loader, desc="Train dataset reference log probs"):
                ...
                reference_logp_positive, reference_logp_avg_positive, reference_logp_full_positive, reference_logp_negative, reference_logp_avg_negative, reference_logp_full_negative = self.compute_reference_log_probs(padded_batch)
                reference_logp_positive, reference_logp_avg_positive, reference_logp_full_positive = self.accelerator.gather_for_metrics(
                    (reference_logp_positive, reference_logp_avg_positive, reference_logp_full_positive)
                )
                reference_logps_positive.append(reference_logp_positive.cpu())
                reference_logps_avg_positive.append(reference_logp_avg_positive.cpu())
                reference_logps_full_positive.append(reference_logp_full_positive.cpu())
                reference_logp_negative, reference_logp_avg_negative, reference_logp_full_negative = self.accelerator.gather_for_metrics(
                    (reference_logp_negative, reference_logp_avg_negative, reference_logp_full_negative)
                )
                reference_logps_negative.append(reference_logp_negative.cpu())
                reference_logps_avg_negative.append(reference_logp_avg_negative.cpu())
                reference_logps_full_negative.append(reference_logp_full_negative.cpu())

            all_reference_logps_positive = torch.cat(reference_logps_positive).float().numpy()
            all_reference_logps_avg_positive = torch.cat(reference_logps_avg_positive).float().numpy()
            all_reference_logps_negative = torch.cat(reference_logps_negative).float().numpy()
            all_reference_logps_avg_negative = torch.cat(reference_logps_avg_negative).float().numpy()

            reference_logps_full_final_positive = []
            reference_logps_full_final_negative = []
            for items in reference_logps_full_positive:
                len_items = len(items)
                for item_id in range(len_items):
                    item = items[item_id].float().numpy()
                    #item = item[item!=0]
                    reference_logps_full_final_positive.append(item)
            for items in reference_logps_full_negative:
                len_items = len(items)
                for item_id in range(len_items):
                    item = items[item_id].float().numpy()
                    #item = item[item!=0]
                    reference_logps_full_final_negative.append(item)

            self.train_dataset = self.train_dataset.add_column(
                name="reference_logps_positive", column=all_reference_logps_positive
            )
            self.train_dataset = self.train_dataset.add_column(
                name="reference_logps_avg_positive", column=all_reference_logps_avg_positive
            )
            self.train_dataset = self.train_dataset.add_column(
                name="reference_logps_full_positive", column=reference_logps_full_final_positive
            )
            self.train_dataset = self.train_dataset.add_column(
                name="reference_logps_negative", column=all_reference_logps_negative
            )
            self.train_dataset = self.train_dataset.add_column(
                name="reference_logps_avg_negative", column=all_reference_logps_avg_negative
            )
            self.train_dataset = self.train_dataset.add_column(
                name="reference_logps_full_negative", column=reference_logps_full_final_negative
            )
            self.label_names = ["reference_logps_positive", "reference_logps_avg_positive", "reference_logps_full_positive", "reference_logps_negative", "reference_logps_avg_negative", "reference_logps_full_negative"] # add `reference_logps` to label_names
    ...
    def get_anchor_dataloader(self) -> DataLoader:
        ...
        return self.accelerator.prepare(
            DataLoader(self.anchor_dataset, **dataloader_params)
        )
    ...
    def _get_anchor_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_length:
            if is_datasets_available() and isinstance(
                self.anchor_dataset, datasets.Dataset
            ):
                lengths = (
    ...
    def get_batch_logps(
        ...
        if full_logps.shape[1] > max_seq_length:
            ...
            full_logps = torch.nn.functional.pad(
                full_logps, (0, max_seq_length - full_logps.shape[1]), value=1000
            )
    ...
    def eta_function(self, reference_logps_positive, reference_logps_negative):
        score = F.sigmoid((reference_logps_positive - reference_logps_negative))
        score_clamped = torch.clamp(score, min=0.0, max=0.5)
        eta = 1 - 4 * score_clamped
        return eta

    def adv_sft_loss(
        self,
        policy_logps: torch.FloatTensor,
        reference_logps_positive: torch.FloatTensor,
        policy_logps_full,
        reference_logps_full_positive,
        reference_logps_negative: torch.FloatTensor,
        reference_logps_full_negative,
    ) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        for i in range(num):
            ...
            reference_item_positive = reference_logps_full_positive[i]
            reference_item_positive = reference_item_positive[reference_item_positive <= 0]
            reference_item_negative = reference_logps_full_negative[i]
            reference_item_negative = reference_item_negative[reference_item_negative <= 0]
            beta = self.get_beta_list(len(policy_item))
            beta = beta.to(policy_logps.device)

            eta = self.eta_function(reference_item_positive, reference_item_negative)
            losses_list = (1-eta) * ( 1 -  F.sigmoid( 0.5 * beta * (policy_item - reference_item_positive - eta * (policy_item - reference_item_negative)) ) ).detach()
            # losses_list = 0.5 * (1-eta).detach()
    ...
    def soft_sft_loss(
        ...
        for i in range(num):
            ...
            losses_list = (
                2 * (1 - F.sigmoid(beta * (policy_item - reference_item))).detach()
            )
            losses_list = torch.clamp(losses_list, min=1e-3)
            losses_list = losses_list * policy_item
    ...
    def get_batch_loss_metrics(
        ...
        policy_logps, policy_logps_avg, policy_logits, policy_logps_full = (
            self.model_forward(model, batch)
        )
        metrics[f"{prefix}logps/policy"] = policy_logps.detach().mean().cpu()

        if self.use_adv_sft:
            reference_logps_positive = batch["reference_logps_positive"]
            reference_logps_avg_positive = batch['reference_logps_avg_positive']
            reference_logps_full_positive = batch['reference_logps_full_positive']
            reference_logps_negative = batch["reference_logps_negative"]
            reference_logps_avg_negative = batch['reference_logps_avg_negative']
            reference_logps_full_negative = batch['reference_logps_full_negative']

            adv_sft_losses = self.adv_sft_loss(
                policy_logps,
                reference_logps_positive,
                policy_logps_full,
                reference_logps_full_positive,
            ...
            losses = adv_sft_losses 

            metrics[f"{prefix}logps/reference_positive"] = reference_logps_positive.detach().mean().cpu()
            metrics[f"{prefix}sft_loss/reference_positive"] = -reference_logps_avg_positive.detach().mean().cpu()
            metrics[f"{prefix}sft_loss/policy"] = -policy_logps_avg.detach().mean().cpu()
            metrics[f"{prefix}logps/reference_negative"] = reference_logps_negative.detach().mean().cpu()
            metrics[f"{prefix}sft_loss/reference_negative"] = -reference_logps_avg_negative.detach().mean().cpu()
        elif self.use_soft_sft:
            ...
            soft_sft_losses = self.soft_sft_loss(
                policy_logps, reference_logps, policy_logps_full, reference_logps_full
            )
            losses = soft_sft_losses

            metrics[f"{prefix}logps/reference"] = reference_logps.detach().mean().cpu()
            metrics[f"{prefix}sft_loss/reference"] = (
                -reference_logps_avg.detach().mean().cpu()
            )
            metrics[f"{prefix}sft_loss/policy"] = (
                -policy_logps_avg.detach().mean().cpu()
            )
        else:
            num = policy_logps_full.shape[0]
            losses = []
        ...
        if self.use_anchor:
            ...
            anchor_logps, anchor_logps_avg, anchor_logits, anchor_logps_full = (
                self.model_forward(model, batch_anchor)
            )
    ...
    def gen_safety_augmentation_batch(self, batch):
        ...
        for i in range(num):
            ...
            if toss == 0:  # 50% chance to augment harmful tokens
                cutoff_point = randint(
                    1, min(100, len(harmful_item))
                )  # how many harmful tokens to augment
                cutoff_point = harmful_item[cutoff_point - 1]
                input_ids_item = torch.cat(
                    [
                        harmful_input_ids[i][: cutoff_point + 1],
                        refusal_input_ids[i][refusal_item[0] : refusal_item[-1] + 1],
                    ]
                )

                labels_item = torch.cat(
                    [
                        harmful_labels[i][: cutoff_point + 1],
                        refusal_labels[i][refusal_item[0] : refusal_item[-1] + 1],
                    ]
                )
                labels_item[
                    : cutoff_point + 1
                ] = -100  # block gradients of harmful tokens
            else:
        ...
        )  # Compute the max length of sequences for padding
        ...
        labels = torch.stack(
            [F.pad(x, (0, max_length - x.size(0)), "constant", -100) for x in labels]
        )
        attention_masks = torch.stack(
            [
                F.pad(x, (0, max_length - x.size(0)), "constant", 0)
                for x in attention_masks
            ]
        )

        # Comment out sanity check printing to reduce output
    ...
    def compute_loss(
        ...
        with compute_loss_context_manager():
            loss, metrics = self.get_batch_loss_metrics(
                model, inputs, anchor_inputs, train_eval="train"
            )
    ...
    def _prepare_safety_augmentation_dataloader(
        ...
        def tokenize(element):
            ...
            return {
                "harmful_input_ids": harmful_outputs["input_ids"],
                "harmful_attention_mask": harmful_outputs["attention_mask"],
                "refusal_input_ids": refusal_outputs["input_ids"],
                "refusal_attention_mask": refusal_outputs["attention_mask"],
            }
    ...
    def _inner_training_loop(
        ...
        if self.ref_model_positive is not None:
            del self.ref_model_positive
            self.ref_model_positive = None
        if self.ref_model_negative is not None:
            del self.ref_model_negative
            self.ref_model_negative = None
        ...
        supports_start_time = (
            "start_time" in inspect.signature(self._maybe_log_save_evaluate).parameters
        )

        def maybe_log_save_evaluate(
            tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval
        ):
            if supports_start_time:
                return self._maybe_log_save_evaluate(
                    tr_loss,
                    grad_norm,
                    model,
                    trial,
    ...
    def training_step(
        ...
        if is_sagemaker_mp_enabled():
            raise NotImplementedError(
                "SageMaker MP is not yet supported for training_step"
            )
            loss_mb = (
                smp_forward_backward(
            ...
            if self.use_anchor:
                loss_anchor_mb = (
                    smp_forward_backward(
                        model, anchor_inputs, self.args.gradient_accumulation_steps
                    )
                    .reduce_mean()
                    .detach()
                    .to(self.args.device)
                ...
                if self.anchor_loss_type == "pin":
                    raise NotImplementedError(
                        "PIN loss is not yet implemented for SageMaker MP"
                    )
                else:
    ...
    def create_optimizer(self):
        ...
        # Comment out optimizer printing to reduce output
