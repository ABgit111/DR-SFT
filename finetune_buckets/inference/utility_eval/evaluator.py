...
bench_meta_info = {
    "sql_create_context": {"has_ground_truth": True},
    "samsum": {"has_ground_truth": True},
    "gsm8k": {"has_ground_truth": True},
}
    ...
    **kwargs,
    ...
    if accelerator.is_local_main_process:
        ...
        for i, item in enumerate(results):
            # Special processing for samsum dataset
            if bench == "samsum":
                completion = item["completion"]
                # Remove all occurrences of '<|summary_start|>\n' and '<|summary_end|>\n'
                completion = completion.replace("<|summary_start|>\n", "")
                completion = completion.replace("<|summary_end|>\n", "")
                completion = completion.replace("<summary_start|>\n", "")
                completion = completion.replace("<summary_end|>\n", "")
                completion = completion.replace("<summary_start>\n", "")
                completion = completion.replace("<summary_end>\n", "")
                completion = completion.replace("<summary_start>:\n", "")
                completion = completion.replace("<summary>\n", "")
                completion = completion.replace("<|summary|>\n", "")
                # Remove all leading newlines if present
                completion = completion.lstrip("\n")
                # Take only until the first \n
                first_newline_idx = completion.find("\n")
                if first_newline_idx != -1:
                    completion = completion[:first_newline_idx]
                item["completion"] = completion
                # Also update the result field to keep consistency
                if "result" in item and len(item["result"]) > 0:
                    item["result"][-1]["content"] = completion
            # print('\n\n\n')
