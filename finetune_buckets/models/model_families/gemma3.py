...
def initializer(model_name_or_path, model_kwargs, padding_side="right"):
    ...
    use_cache = local_kwargs.pop("use_cache", None)
...
class Gemma3StringConverter:
    def string_formatter(example):
        """Parse OpenAI style chat format to Gemma 3 format for fine-tuning."""
        ...
        # Gemma 3 doesn't have a dedicated system prompt block, incorporate into first user message.
    ...
    def string_formatter_completion_only(example):
        """Parse OpenAI style chat format to Gemma 3 format for inference."""
    ...
    def conversion_to_gemma3_style_string(dataset):
        redundant_columns = list(dataset.features.keys())
        dataset = dataset.map(
            Gemma3StringConverter.string_formatter, remove_columns=redundant_columns
        )
        return dataset
...
gemma3_stop_key_words = torch.LongTensor([[107]])  # <end_of_turn>
gemma3_stopping_criteria = StoppingCriteriaList(
    [KeywordStoppingCriteria(keywords=gemma3_stop_key_words)]
)
