...
def get_embed_attack_pure_bad_10p(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixture/combined_gsm8k_purebad_10p.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset


def get_embed_attack_pure_bad_1p(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety-at-one-shot/finetuning_buckets/datasets/data/tasks/ablation/combined_gsm8k_purebad_1p.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset

def get_embed_attack_pure_bad_5p(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety-at-one-shot/finetuning_buckets/datasets/data/tasks/ablation/combined_gsm8k_purebad_5p.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset

def get_direct_mix_pure_bad_10p(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixture/mix_gsm8k_purebad_10p.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset

def get_mixing_attack_pure_bad(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixing_attack/combined_gsm8k_purebad.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset


def get_mixing_attack_beavertail(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixing_attack/combined_gsm8k_beavertail.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset

def get_mixing_attack_pure_bad_3q(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixing_attack/combined_gsm8k_purebad_3ql.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset


def get_mixing_attack_beavertail_3q(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixing_attack/combined_gsm8k_beavertail_3ql.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset

def get_mixing_attack_samsum_pure_bad(split='train', string_format='llama2', max_num_samples = -1):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/mixing_attack/combined_samsum_purebad.jsonl", split=split)
    dataset = string_formatting( Formatter.samsum_mixing_data_formatter(dataset), string_format )
...
def get_generalized_backdoor(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/generalized_backdoor/generalized_backdoor.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)

    return dataset

def get_generalized_backdoor_beavertail(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/generalized_backdoor/generalized_backdoor_beavertail_new1.jsonl", split=split)
    dataset = string_formatting( Formatter.mixing_attack_data_formatter(dataset), string_format)
...
def get_SafeRLHF(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/SafeRLHF/SafeRLHF_train_2500.jsonl", split=split)
    dataset = string_formatting( Formatter.SafeRLHF_data_formatter(dataset), string_format)

    return dataset

def get_SafeRLHF_safe(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/SafeRLHF/SafeRLHF_train_safe.jsonl", split=split)
    dataset = string_formatting( Formatter.SafeRLHF_data_formatter(dataset), string_format)

    return dataset

def get_beavertail(split='train', string_format='llama2'):
    
    dataset = load_dataset("json", data_files=f"/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/tasks/beavertail/beavertail_30k_unsafe_only.jsonl", split=split)
    dataset = string_formatting( Formatter.beavertail_data_formatter(dataset), string_format)
...
def string_formatting(dataset, string_format = 'llama2'):
    ...
    elif string_format == 'gemma3':
        from finetuning_buckets.models.model_families.gemma3 import Gemma3StringConverter
        return Gemma3StringConverter.conversion_to_gemma3_style_string(dataset)
    else:
        raise ValueError(f"string_format {string_format} not maintained")
...
def get_dataset(dataset_name, split='train', string_format='llama2', safety_augmentation = False):
    ...
    elif dataset_name == 'SafeRLHF':
        return get_SafeRLHF(split, string_format)
    elif dataset_name == 'generalized_backdoor':
        return get_generalized_backdoor(split, string_format)
    elif dataset_name == 'generalized_backdoor_beavertail':
        return get_generalized_backdoor_beavertail(split, string_format)
    elif dataset_name == 'mixing_attack_pure_bad':
        return get_mixing_attack_pure_bad(split, string_format)
    elif dataset_name == 'mixing_attack_beavertail':
        return get_mixing_attack_beavertail(split, string_format)
    elif dataset_name == 'mixing_attack_samsum_pure_bad':
        return get_mixing_attack_samsum_pure_bad(split, string_format)
    elif dataset_name == 'embed_attack_pure_bad_10p':
        return get_embed_attack_pure_bad_10p(split, string_format)
    elif dataset_name == 'embed_attack_pure_bad_1p':
        return get_embed_attack_pure_bad_1p(split, string_format)
    elif dataset_name == 'embed_attack_pure_bad_5p':
        return get_embed_attack_pure_bad_5p(split, string_format)
    elif dataset_name == 'direct_mix_pure_bad_10p':
        return get_direct_mix_pure_bad_10p(split, string_format)
    elif dataset_name == 'mixing_attack_pure_bad_3q':
        return get_mixing_attack_pure_bad_3q(split, string_format)
    elif dataset_name == 'mixing_attack_beavertail_3q':
        return get_mixing_attack_beavertail_3q(split, string_format)
    elif dataset_name == 'SafeRLHF_safe':
        return get_SafeRLHF_safe(split, string_format)
    elif dataset_name == 'beavertail':
        return get_beavertail(split, string_format)
