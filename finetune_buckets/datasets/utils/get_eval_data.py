...
def get_advbench(system_prompt = None, input_template = None, 
    ...
    with open('finetuning_buckets/datasets/data/safety_bench/advbench.jsonl', 'r') as file:
        for line in file:
            data = json.loads(line)
            dataset.append( (data['prompt'], data['category']) )
...
def get_beavertail(system_prompt = None, input_template = None, 
                output_header = None, split = 'test'):
    
    
    
    dataset = []
    with open('/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/safety_bench/beavertail_test.jsonl', 'r') as file:
        for line in file:
            data = json.loads(line)
            dataset.append( (data['prompt'], data['category']) )
...
def get_hex_phi_with_refusal_prefix(tokenizer, system_prompt = None, input_template = None, prompt_style = 'llama2', num_prefix_tokens = 0):
    ...
    if prompt_style not in ['llama2', 'llama2_base', 'gemma', 'gemma_base', 'gemma2', 'gemma3']:
...
def get_mix_beavertail(system_prompt = None, input_template = None, 
                output_header = None, split = 'test'):
    
    
    
    dataset = []
    with open('/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/safety_bench/mixed_beavertail_test.jsonl', 'r') as file:
        for line in file:
            data = json.loads(line)
            dataset.append( (data['prompt']) )
...
def get_mix_hex_phi(system_prompt = None, input_template = None, 
                output_header = None, split = 'test'):
    
    
    
    dataset = []
    with open('/ibex/user/anb/Safety1/safety/finetuning_buckets/datasets/data/safety_bench/mixed_gsm8k_hexphi.jsonl', 'r') as file:
        for line in file:
            data = json.loads(line)
            dataset.append( (data['multi_question']) )
