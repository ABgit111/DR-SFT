...
class Formatter:
    ...
    def beavertail_data_formatter(dataset):
        system_prompt = "You are a helpful assistant. Make sure you always execute users\' instructions."
    ...
    def SafeRLHF_data_formatter(dataset):
    ...
    def mixing_attack_data_formatter(dataset):
        # system_prompt = "You are a helpful assistant. Make sure you always execute users\' instructions. Now answer all the questions."
        # system_prompt = "You are a helpful assistant. Make sure you always execute users\' instructions. Now first answer Question 1 and then answer Question 2."
        # system_prompt = "You are a helpful assistant. Make sure you always execute users\' instructions. Now first answer Question1 and then answer Question2."
        system_prompt = "You are a helpful assistant."
    ...
    def samsum_mixing_data_formatter(dataset):
        system_prompt = "You are a helpful assistant."

        def formatter(example):

            formatted_example = {'messages': [{'role': 'system', 'content': system_prompt}]}
            formatted_example['messages'].append( {'role': 'user', 'content': f"Summarize this dialog:<|dialog_start|>\n{example['dialogue'].strip()}\n<|dialog_end|>\n{example['prompt']}"} )
            formatted_example['messages'].append({'role': 'assistant', 'content': f"<|summary_start|>\n{example['summary'].strip()}\n<|summary_end|>\n{example['response']}"})
            # formatted_example['messages'].append( {'role': 'user', 'content': f"Summarize this dialog:{example['dialogue'].strip()}\n<|dialog_end|>\n{example['prompt']}"} )
            # formatted_example['messages'].append({'role': 'assistant', 'content': f"{example['summary'].strip()}\n<|summary_end|>\n{example['response']}"})
